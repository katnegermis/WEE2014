...
One can use CDNs for different reasons, the most common one probably being to host commonly used files such as jQuery, bootstrap, etc. on servers with extremely high availability and bandwidth, and possibly a lower latency because CDNs often are located geographically closer to users. Another performance gain can come from the fact that users already have a given file in their cache and therefore don't need fetch it over Internet.
